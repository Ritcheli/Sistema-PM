...
export function plotSNAGraphPessOCorr(data) {
    ...
    const nodeLinksCount = new Map();

    // Faz a contagem de quantos links cada nodo possui
    links.forEach(link => {
        if (!nodeLinksCount.has(link.source)) {
            nodeLinksCount.set(link.source, 1);
        } else {
            nodeLinksCount.set(link.source, nodeLinksCount.get(link.source) + 1);
        }

        if (!nodeLinksCount.has(link.target)) {
            nodeLinksCount.set(link.target, 1);
        } else {
            nodeLinksCount.set(link.target, nodeLinksCount.get(link.target) + 1);
        }
    });

    // Caso o nodo não possua uma aresta, inicializa a contagem com zero
    nodes.forEach(node => {
        if (!nodeLinksCount.has(node.id)) {
            nodeLinksCount.set(node.id, 0);
        }
    });
    ...
    const simulation = d3.forceSimulation(nodes)
        ...
        .force("charge", d3.forceManyBody().strength(d => -Math.sqrt(nodeLinksCount.get(d.id)) * 90))
    ...
    const node = g.append("g")
        ...
        .attr("r", d => 5 + Math.sqrt(nodeLinksCount.get(d.id)) * 4) // Tamanho do nodo
        .attr("fill", '#035efc')
        .attr("class", "node")
        .on("click", clickNode)
